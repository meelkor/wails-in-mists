...
func update_animation(state: AnimationState) -> void:
	# also ugly
	if character_scene and character_scene.supports_animations and "parameters/State/transition_request" in character_scene.animation_tree:
...
func _apply_final_velocity(v: Vector3) -> void:
	...
	if character_scene.supports_animations:
		...
		if "parameters/CombatWalkBlend/blend_amount" in character_scene.animation_tree:
