...
func _ready() -> void:
	_init_current_child()
	child_entered_tree.connect(func (_n: Node) -> void: _update_child_size())
...
func _init_current_child() -> void:
	...
	if child:
		_update_child_size()
		child.size_flags_changed.connect(func () -> void: _update_child_size())
